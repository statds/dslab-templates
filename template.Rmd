...
consistent with the syclit of `r latex`. For instance, $x_i^2$, $\sin(x)$,
...
Package `r pkg("pander")` [@daroczi2015pander] provides unctions printing a
...
```{r pythonCode, eval = TRUE, engine = "python", comment = ">>> "}
foo = "Hello " + "world!"
print("'" + foo + "'" + ' has %d letters.' % len(foo))
```
...
In summary, we provided the project template and reviewed the basic syntax of
writing a single `r proglang("R Markdown")` document with the power and love of
`r pkg("bookdown")` at the same time.
